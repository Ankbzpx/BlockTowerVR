...
public class CustomMaterialInspector : MaterialEditor {
	...
	public override void OnInspectorGUI()
	{
		...
		if (isVisible && !theShader.hasMultipleDifferentValues && theShader.objectReferenceValue != null )
		{
			...
			if (mat.GetFloat ("_RealTimeReflection") == 1) {
				reflections = true;
			} else {
				reflections = false;
			}
			...
			if (customEditor) {
				...
				GUILayout.BeginHorizontal ();
				Texture reflectionsicon = Resources.Load ("Icons/reflections") as Texture;
				GUILayout.Label (reflectionsicon,labelStyle());
				if (foldButton (s4))
					s4 = !s4;
				GUILayout.EndHorizontal ();
				if(s4)
			...
			GUILayout.Box("Stylized Water Shader created by Marc Sureda, 2017");
		}
	}
	...
	public void ColorGradientSection(){
		...
		Color c1 = EditorGUILayout.ColorField (GUIContent.none, mat.GetColor ("_DepthGradient1"), false, false, false, hdr, GUILayout.MaxWidth(50),GUILayout.MinHeight(30));
		Color c2 = EditorGUILayout.ColorField (GUIContent.none, mat.GetColor ("_DepthGradient2"), false, false, false, hdr, GUILayout.MaxWidth(50),GUILayout.MinHeight(30));
		Color c3 = EditorGUILayout.ColorField (GUIContent.none, mat.GetColor ("_DepthGradient3"), false, false, false, hdr, GUILayout.MaxWidth(50),GUILayout.MinHeight(30));
		...
		GUILayout.BeginHorizontal ();
		Color fr = EditorGUILayout.ColorField (GUIContent.none, mat.GetColor ("_FresnelColor"), false, false, false, hdr, GUILayout.MaxWidth(50),GUILayout.MinHeight(20));
	...
	public void FoamSection(){
		...
		ColorPickerHDRConfig hdr = new ColorPickerHDRConfig(0,1,0,1);
		Color foamcolor = EditorGUILayout.ColorField (GUIContent.none, mat.GetColor ("_FoamColor"), false, false, false, hdr, GUILayout.MaxWidth(50),GUILayout.MinHeight(20));
	...
	public void ReflectionsSection (){
		reflections = EditorGUILayout.Toggle ("Real time reflections", reflections);
		if (reflections) {
			mat.SetFloat ("_RealTimeReflection", 1);
		} else {
			mat.SetFloat ("_RealTimeReflection", 0);
		}

		if (!mat.GetTexture ("_ReflectionTex") && reflections) {
			EditorGUILayout.HelpBox ("You need the WaterReflection Script in this GameObject in order to have real time reflections", MessageType.Error);
			reflections = false;
		}
			
		//if (reflections) {
		//	RangeProperty("_ReflectionsIntensity", "Intensity",0,3);
